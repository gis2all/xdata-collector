...
当前正式支持 `Windows / Linux / macOS` 三端原生运行，以及 Docker 运行。文档里的标准启动顺序是 `python doctor.py` -> `python install.py` -> `python services.py start`。
...
默认开发机当前仍是 Windows，但代码和运行说明要按三端一致口径维护。`services.py` 默认管理 API、Scheduler、Dev UI，并优先使用 `psutil` 统一处理 PID、端口和进程树；`run/static_web_server.py` 是构建产物预览入口。
...
- Dockerfile 已固定：
  ...
  - `xreach-cli@0.3.0`
- `run/bootstrap.py` 本机安装的 `twitter-cli` 和 `xreach-cli` 必须与 Dockerfile 固定到同一版本；当前同时安装 `psutil`，改版本时要同步 Dockerfile、bootstrap 和测试。
- Docker Compose 现在改成代理显式 opt-in：未设置 `DOCKER_PROXY_URL` 时不注入代理环境变量；需要代理时再显式设置它。
